...
**Claude Code・Codex・Cursor・Gemini・qoder・opencode** を、統制された多段階コーディングパイプラインへとまとめ上げるデスクトップコックピット。計画承認ゲート、ネイティブセッションのインポート、リアルタイムの使用量トラッキング、MCP 連携、そして作業に寄り添うデスクトップペット付き。
...
- **🎛️ マルチエージェント編成** —— 各ワークフロー段階を、異なるコーディング CLI（Claude Code・Codex・Cursor・Gemini・qoder・opencode）と異なるモデルに振り分け。**opencode** はそれ自体がマルチプロバイダのゲートウェイ——一度つなぐだけで多数のモデルベンダーに到達できます。
- **📂 外部エディタで開く** —— タイトルバーの「場所を開く」ボタンがインストール済みエディタ（VS Code・Cursor・JetBrains・Zed・Finder・ターミナル…）を検出し、現在のワークスペース——またはプレビュー中のファイル——を選んだアプリで開き、既定として記憶します。
- **⌨️ チャットのスラッシュコマンド** —— チャットで `/` を入力すると、ワークフロー起動に加え、**ローカルの実際のカスタムコマンド/プロンプトとインストール済みスキル**をエージェントごとに絞り込んで一覧表示。
...
│   ├── agents/    # CLI アダプター（claude・codex・cursor・gemini・qoder・opencode）+ providers
