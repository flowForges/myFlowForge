...
> ⚠️ **项目状态：** 一个持续开发中的个人项目，目标平台是 **macOS**（Apple Silicon 与 Intel）。基于 Electron，其他平台可从源码构建，但目前只打 macOS 包。发布的多为 beta 版本 —— 新功能都在 beta 通道里。
...
- **MCP 反向桥** —— 内置 Forge MCP 服务器让代理反过来调用应用：`forge_ask`、`forge_propose_plan`、`forge_write_artifact`、`forge_handoff`、`forge_delegate`。注入给支持 MCP 的 8 个代理，其余走文字指令兜底。
...
- **个性化** —— 6 套原创皮肤、12 种强调色、内置壁纸画廊或自己的图，应用与对话区可分别设精确 px 字号，深浅两套配色分别调过对比度。
