...
// ── base64 / UTF-8 编解码 ─────────────────────────────────────────────────────
// ★实现搬到了 `./base64`(**零依赖**那份),因为手机端也要用它们做粘贴内容的编码,
//  而这份文件第一行 `import nacl from 'tweetnacl'` —— 从这里拿会把整个加密库拖进手机端 bundle,
//  可 `tweetnacl` 根本不在 `mobile/package.json` 里。搬走的理由在 `base64.ts` 顶部写着。
//  这里原样 re-export:原有的调用方和 e2e 自己的测试一个字都不用改。
import { toBase64, fromBase64, utf8ToBytes, bytesToUtf8 } from './base64'
export { toBase64, fromBase64, utf8ToBytes, bytesToUtf8 }
