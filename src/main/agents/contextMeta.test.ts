...
describe('runtime agent context extraction', () => {
  it('extracts actual skill and rule paths from streamed thinking/tool text', () => {
    ...
    const text = [
      '调用 shell: /bin/zsh -lc "sed -n \'1,220p\' /Users/you/.codex/skills/using-superpowers/SKILL.md"',
    ...
    expect(meta.skills).toEqual([
      expect.objectContaining({ name: 'using-superpowers', path: '/Users/you/.codex/skills/using-superpowers/SKILL.md', state: 'ok' }),
    ])
