...
export const STAGE_PROMPTS: Record<StageKey, string> = {
  ...
  design: '基于需求产出技术方案:模块划分、接口/数据结构设计、关键技术决策与替代方案,并评估技术风险与影响面。**探查(重要):上游若给了【关联清单】,就从清单里的文件/模块出发重点阅读,必要时再顺藤摸瓜——不要对整个仓库重新穷尽式通读(那是重复劳动,又慢又耗 token)。判据是"能说清方案影响面"即可。** **各项目分工(必须,当本工作流涉及多个项目/仓库时——项目列表见对话中告知):技术方案里必须包含一节「## 各项目任务分工」,在其下为每个项目单列一个 `### <项目名>` 子节。每个子节要让用户一眼看清该项目要做什么——用清晰可扫读的结构:开头一句『目标：…』,随后用要点列表逐条写具体改动(每条=做什么 + 涉及的文件/模块/接口),必要时标注依赖或先后顺序。整体方案与各项目分工都放在同一份文档里(不要拆成多个文件),因为代码阶段每个项目的 agent 会读这同一份完整方案、并定位到自己那一节。** **无论你用什么方法或 spec(自由发挥、各类计划/规格流程等)来推演方案,最终都必须把完整方案(含上面的各项目分工)汇总落到这同一份技术方案文档里;不要让交付物散落到其他文件/目录,否则代码阶段读不到。** **产出方式(必须):把完整技术方案写成一个 markdown 文件,并调用 forge_write_artifact 登记它(kind=md)——不要只把方案正文写在回复里,否则下游拿不到方案。**',
