...
describe('fetchLatestRelease', () => {
  it('parses tag, notes, and the .dmg asset from the list', async () => {
    ...
    const info = await fetchLatestRelease('o/r', { fetch: fakeFetch(true, [RELEASE]) , platform: 'darwin' })
    expect(info).toEqual({
  ...
  it('picks the highest semver even when a lower version is listed first', async () => {
    ...
    const info = await fetchLatestRelease('o/r', { fetch: fakeFetch(true, list) , platform: 'darwin' })
  ...
  it('returns null when there is no installable asset', async () => {
    const info = await fetchLatestRelease('o/r', { fetch: fakeFetch(true, [{ tag_name: 'v2.4.0', assets: [] }]) , platform: 'darwin' })
    expect(info).toBeNull()
  })
  it('returns null when there are no stable releases', async () => {
    const info = await fetchLatestRelease('o/r', { fetch: fakeFetch(true, []) , platform: 'darwin' })
    expect(info).toBeNull()
  })
  ...
  it('THROWS on a non-ok response (404 / rate limit)', async () => {
    await expect(fetchLatestRelease('o/r', { fetch: fakeFetch(false, {}), attempts: 1 , platform: 'darwin' })).rejects.toThrow()
  })
  it('THROWS when fetch throws (offline)', async () => {
    await expect(fetchLatestRelease('o/r', { fetch: async () => { throw new Error('offline') }, attempts: 1 , platform: 'darwin' })).rejects.toThrow('offline')
  })
  it('RETRIES a transient failure and succeeds (does not falsely report 检查失败)', async () => {
    ...
    const info = await fetchLatestRelease('o/r', { fetch: flaky as any, retryDelayMs: 0 , platform: 'darwin' })
    expect(calls).toBe(3)
  ...
  it('picks the arm64 dmg for an arm64 machine', async () => {
    const info = await fetchLatestRelease('o/r', { fetch: fakeFetch(true, MULTI), arch: 'arm64' , platform: 'darwin' })
  ...
  it('picks the x64 dmg for an x64 machine (not the first asset)', async () => {
    const info = await fetchLatestRelease('o/r', { fetch: fakeFetch(true, MULTI), arch: 'x64' , platform: 'darwin' })
  ...
  it('falls back to a universal dmg when no arch-specific build exists', async () => {
    ...
    const info = await fetchLatestRelease('o/r', { fetch: fakeFetch(true, uni), arch: 'arm64' , platform: 'darwin' })
