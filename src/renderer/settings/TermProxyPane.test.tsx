...
describe('TermProxyPane', () => {
  it('does NOT prefill an unsaved default — an empty setting shows an empty box (no decoy)', () => {
    ...
    render(<TermProxyPane title="终端代理" desc="" scope="agent" termProxy="" onChange={onChange} />)
    ...
    const { container } = render(<TermProxyPane title="终端代理" desc="" scope="agent" termProxy="" onChange={vi.fn()} />)
  ...
  it('shows the active proxy in the current-state line when set', () => {
    render(<TermProxyPane title="终端代理" desc="" scope="agent" termProxy="http://127.0.0.1:1080" onChange={vi.fn()} />)
  ...
  it('检测出口 IP button shows a failure message on error', async () => {
    ...
    render(<TermProxyPane title="终端代理" desc="" scope="agent" termProxy="" onChange={vi.fn()} />)
    fireEvent.click(screen.getByText('检测出口 IP'))
  ...
  it('edits the proxy and reports changes; reset clears it', () => {
    const onChange = vi.fn()
    render(<TermProxyPane title="终端代理" desc="" scope="agent" termProxy="http://127.0.0.1:7897" onChange={onChange} />)
    const input = screen.getByPlaceholderText('http://127.0.0.1:7897') as HTMLInputElement
  ...
  describe('保存状态必须如实', () => {
    const setup = (termProxy: string) => {
      const onChange = vi.fn()
      render(<TermProxyPane title="终端代理" desc="" scope="agent" termProxy={termProxy} onChange={onChange} />)
