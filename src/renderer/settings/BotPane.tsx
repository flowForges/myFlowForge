...
const STATUS_LABEL: Record<BotStatus['state'], string> = {
  offline: '未连接', connecting: '连接中…', online: '在线', error: '错误',
}
const STATUS_COLOR: Record<BotStatus['state'], string> = {
  offline: 'var(--faint)', connecting: 'var(--warn, #d90)', online: 'var(--ok, #2a7)', error: 'var(--del, #c33)',
}
  ...
  return (
    <div className="mem-pane">
      <div className="set-group">
        ...
        <div className="bot-status" style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '8px 0' }}>
          <span style={{ width: 9, height: 9, borderRadius: '50%', background: STATUS_COLOR[status.state] }} />
          <span>{STATUS_LABEL[status.state]}</span>
          {status.state === 'error' && <span style={{ color: 'var(--del)' }}>：{status.reason}</span>}
        </div>
      </div>
      ...
      <div className="set-group">
        <label className="bot-field">
          <span>Client ID</span>
          <input value={clientId} onChange={e => setClientId(e.target.value)} placeholder="dingxxxxxxxx" spellCheck={false} />
        </label>
        <label className="bot-field">
          <span>Client Secret</span>
          <input type="password" value={clientSecret} onChange={e => setClientSecret(e.target.value)} placeholder="••••••••" spellCheck={false} />
        </label>
        ...
        {credsDirty && <p className="mem-switch-hint" style={{ color: 'var(--warn)' }}>凭据已修改，先「保存凭据」再连接。</p>}
      </div>

      <div className="set-group">
        <h4>配对</h4>
        <p className="mem-switch-hint">在钉钉里私聊机器人，发送 <b>bind {config.pairingCode || '——'}</b> 完成绑定（配对码单次有效，绑定后自动更换）。</p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <code style={{ fontSize: 20, letterSpacing: 3, fontWeight: 600 }}>{config.pairingCode || '——'}</code>
      ...
      <div className="set-group">
        <h4>推送粒度</h4>
        {VERBOSITY.map(v => (
          <label key={v.key} className="bot-radio" style={{ display: 'flex', gap: 8, alignItems: 'flex-start', margin: '6px 0' }}>
            <input type="radio" name="bot-verbosity" checked={config.verbosity === v.key}
              onChange={() => onChange({ ...config, verbosity: v.key })} />
            <span><b>{v.label}</b><br /><span className="mem-switch-hint">{v.hint}</span></span>
          </label>
        ))}
      </div>
      ...
      <div className="set-group">
        ...
        {config.bindings.length === 0
          ? <div className="proj-empty">还没有绑定。按上面的配对步骤在钉钉里绑定。</div>
          : config.bindings.map(b => (
            <div key={b.chatId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--line)' }}>
              <div>
                <div>{b.chatType === 'private' ? '私聊' : '群'} · {b.userId || b.chatId}</div>
                <div className="mem-switch-hint">焦点：{b.focus ? `${b.focus.sessionId}` : '未设置（在钉钉发 list / attach）'}</div>
              </div>
      ...
      <div className="set-group">
        <h4>钉钉里可用指令</h4>
        <p className="mem-switch-hint" style={{ whiteSpace: 'pre-line' }}>{[
          'list — 列出工作区与会话',
          'attach <id> — 选定焦点会话（如 attach s1）',
          'status — 查看当前焦点与待处理',
          'stop — 停止当前焦点会话',
          'unbind — 解绑本聊天',
          '有待处理门时直接回复即作答（allow/deny、序号、或文本）；否则直接发消息 = 给焦点会话开新对话。',
        ].join('\n')}</p>
      </div>
    </div>
  )
