...
export function useChat(
  ...
  useEffect(() => {
    const off = api.current.onChatEvent((e: ChatEvent) => {
      ...
      else if (e.type === 'plan-request') setPlans(p => [...p, { id: e.id, approach: e.approach, stages: e.stages, hooks: e.hooks, allProjects: e.allProjects, task: e.task, workflowId: e.workflowId, workflowName: e.workflowName, workflowOptions: e.workflowOptions, ts: new Date().toISOString() }])
