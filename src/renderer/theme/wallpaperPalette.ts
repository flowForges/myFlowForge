...
// ★次级文字(fg-2 / muted / faint)不取 tokens.css 的基础值,取 6 套内置皮肤真机调优后的值。
// 原因:自动配色永远伴随壁纸,而壁纸模式下 chrome 面板是半透明的,亮壁纸会把面板顶亮 —— 基础深色的
// muted 64 / faint 50 在那种底上直接读不出(皮肤系统那轮就是这么踩过来的,4 套深色皮肤最终落在
// muted 75.1–79.2、faint 63.3–67.3、fg-2 83.1–86.8)。浅色同理反向:ink/mossgarden 的 faint 是
// 59.1/60.1,比基础值 62 更深,因为浅底上太淡就是失明。
const DARK_LADDER: Rung[] = [
  ['--bg', 20, 1.00], ['--bg-2', 23, 1.09], ['--sidebar', 17, 1.18],
  ['--surface', 26, 1.09], ['--surface-2', 30, 1.18],
  ['--fg', 95, 0.36], ['--fg-2', 85, 0.55], ['--muted', 76, 0.82], ['--faint', 64, 0.82],
]

const LIGHT_LADDER: Rung[] = [
  ['--bg', 98, 1.00], ['--bg-2', 96, 1.20], ['--sidebar', 95, 1.40],
  ['--surface', 99, 0.70], ['--surface-2', 96.5, 1.50],
  ['--fg', 24, 0.50], ['--fg-2', 34, 0.50], ['--muted', 50, 0.50], ['--faint', 59, 0.40],
]
