...
const byListeners: ((p: { by: string }) => void)[] = []
...
beforeEach(() => {
  ...
  ;(window as unknown as { forge: unknown }).forge = {
    ...
    onSettingsChangedBy: (cb: (p: { by: string }) => void) => { byListeners.push(cb); return () => {} },
...
describe('RemoteBar', () => {
  ...
  it('★别的设备改了这台机器的设置时,说清是谁改的(Q7)', async () => {
    // 「后写的赢」本身没问题,问题是另一端会看到值凭空变了、界面跟着变了,像是自己点错了。
    render(<RemoteBar onOpenHosts={() => {}} />)
    await push({ hostId: 'h1', label: '云服务器', state: { status: 'ready', version: '1.1.2', methods: [] }, methods: [] })
    await act(async () => { byListeners.forEach((l) => l({ by: 'iPhone' })) })
    expect(screen.getByText(/「iPhone」刚改了这台机器的设置/)).toBeInTheDocument()
  })
})
