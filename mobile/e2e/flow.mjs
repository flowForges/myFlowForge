...
// ④ ★从主机列表切回会话 tab,必须落在会话列表,**不能落进一张添加页**。
//
//    ★★2026-08-29 tab 化之后这条的走法变了:主机屏是**底部 tab 的一格**,不再是被推进来的
//    次级屏,所以它没有 `‹`(见 `app/(tabs)/hosts.tsx` 顶上的注释)。「回会话列表」现在是
//    切 tab,不是退栈 —— 断言改成点「会话」这枚 tab 标签。
//
//    ★★先说清楚这条**没有**覆盖什么,免得半年后有人看着它以为用户报的那个 bug 已经被钉住了:
//    用户真机上撞见的是**扫码**那条路 —— 「扫一扫」是 `push('/scan')`,`Scanner` 解完码是
//    `replace('/add-host', 参数)`(replace 换掉的是 `/scan` 那一层),于是栈里留下**两个**
//    `/add-host`,当时保存完走 `goBack()` 只弹掉带参那个,人落回下面那张空的添加页。
//    **这个成因在本套件里造不出来**:无头 Chrome 没有 BarcodeDetector,`add-host.tsx` 据此
//    连「扫一扫」那颗按钮都不摆;上面走的首跑流程(引导屏 → 添加主机)栈里也只压得出**一个**
//    `/add-host`。真正的复现只有真机(见 `docs/superpowers/plans/2026-08-26-mobile-ui-phase2-UNVERIFIED.md` §1)。
//    深链冷启动直接落在 `/add-host`(根栈里没有 tabs 打底)那条分支已经在
//    Task 4 复核里用手工脚本单独走过,见 task-4-report.md。
//
//    这条断言现在是个**状态断言**:它不问你怎么来的,只钉「从主机列表切到会话 tab,人落在哪儿」。
//    它挡得住的是**任何**让 `goToHosts()` 落不到真正主机屏的写法(比如把
//    `nav.ts` 的 `goToHosts()` 顺手写残,让它把一张添加页留在 `(tabs)` 下面,tab 切换时那张
//    残留页会在 `会话` tab 内容前面露出来)。这件事**没有任何单元测试看得见**:它整个活在
//    导航栈里。
