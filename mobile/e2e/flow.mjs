...
//
//    这条断言是个**状态断言**:它不问你怎么来的,只钉「从主机列表退一步,人落在哪儿」。
//    它独立挡得住的是**任何**在主机列表底下留了一张添加页的写法 —— 最现实的一种就是把
//    `nav.ts` 的 `goToHosts()`(`dismissTo('/hosts')`)顺手写成 `router.push('/hosts')`。
//    这件事**没有任何单元测试看得见**:它整个活在导航栈里。
//
//    变异验证(两条,都实跑过):
//    · `goToHosts()` → `router.push('/hosts')`:③ 仍 PASS(确实落在主机列表),
//      ④ **FAIL**,extra 打出来的正是 `‹ / 添加主机 / 扫电脑上那枚码,或者手填地址`。
//      —— 这条才是真正验到 ④ 头上的那个变异。
//    · `goToHosts()` → `goBack()`(bug 的原始形态):栈只有一层,直接落回列表屏,
//      **③ 先红**、脚本随后死在找不到 `‹` 上,**④ 根本没被求值** —— 那条变异证明的是
//      ③ 有效,不是 ④ 有效。别拿它给 ④ 记功。
await p.clickText('‹')
