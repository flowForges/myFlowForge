...
在**你的 mac / Windows 上的 app 里**连到这台机器,右下角那个终端面板开出来的就是**这台 Linux 的 shell**
—— 当前目录、`git status`、跑测试,都在服务器上。面板标题栏上会挂一枚芯片写着主机名,
提醒你这不是本机(★两个黑框长得一模一样,那枚芯片是屏幕上唯一能回答「这是哪台机器」的东西)。

要它能用,服务器上得有 node-pty(第三节第 1 步那个 `python3 make g++`)。没装好的话
开终端会明说「这台主机上的终端组件(node-pty)没装好」,而不是点了没反应。

两件跟单机不一样的事:

- 终端**跟着连接走**。断线重连、或者切到别的主机,这台机器上那些 shell 会被就地收掉,
  标签页变成「已退出」。★这是有意的:不收的话,每断一次线就在服务器上留一个永远不会退出的
  shell,几周之后表现成「终端打不开了」,而没人会想到去查几周前的那次断线。
  **要留着跑的长任务用 `tmux` / `screen`**,别指望终端标签页替你留着。
- 多个客户端连同一台机器时,**各看各的**:你开的终端别人看不见,也顶不掉。
...
- **手机上没有终端**。终端本身在这台机器上是通的(见第七节),但手机端没做那个界面 ——
  在手机上敲命令这件事,装个 SSH 客户端比什么都强。桌面端连过来是有终端的。
- 本文档在 **Debian 12 / node 22 / x86_64**(Docker)上实跑过:daemon 起得来、
  `pair` 出的码解得开、从另一台机器直连和经中转两条路都握上手并 invoke 成功(2026-09-02);
  终端整条路走通 —— 开 pty、写命令、读回输出,断线后 pty 被收干净(2026-09-03)。
